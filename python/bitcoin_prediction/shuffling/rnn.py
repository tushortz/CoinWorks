...
def load_data():
    ...
    if (ALL_YEAR_INPUT_ALLOWED):
        ...
        priced_bitcoin = priced_bitcoin[priced_bitcoin['year'] == YEAR].reset_index(
            drop=True)
...
def preprocess_data(data, window_size, prediction_horizon):
    ...
    for current_index, current_row in data.iterrows():
        if (current_index < (window_size + prediction_horizon - 1)):
            ...
            temp = np.concatenate((data[start_index:end_index + 1]['price'].values.reshape(
                1, -1), np.asarray(current_row['price']).reshape(1, 1)), axis=1)
            temp = np.concatenate(
                (temp, np.asarray(current_row['day']).reshape(1, 1)), axis=1)
...
def build_rnn(train_input, train_target, window_size, batch_size):
    ...
    for i in range(STEP_NUMBER):
        model.fit(train_input, train_target, epochs=1,
...
def get_nn(window_size, batch_size):
    model = Sequential()
    model.add(LSTM(UNITS_OF_HIDDEN_LAYER_1, batch_input_shape=(
        batch_size, 1, window_size), stateful=True, return_sequences=True))
    model.add(Dropout(DROP_OUT_PERCENTAGE))
    model.add(LSTM(UNITS_OF_HIDDEN_LAYER_2, batch_input_shape=(
        batch_size, 1, window_size), stateful=True))
    model.add(Dropout(DROP_OUT_PERCENTAGE))
...
def print_model(scaler, rnn, test_X, test_Y, test_days, batch_size, window_size, prediction_horizon):
    myFile = open('./python/results/rnn_order__' + str(YEAR) + ".csv", 'a')
    ...
    previous_price = np.asarray(scaler.inverse_transform(
        test_X.reshape(-1, window_size)[:, -1].reshape(-1, 1)))

    myFile.write("WINDOW_SIZE: " + str(window_size) + "\n")
    myFile.write("PREDICTION_HORIZON: " + str(prediction_horizon) + "\n")

    original_log_return = np.log(np.asarray(
        test_predicted).reshape(-1, 1)/previous_price)

    for i in range(test_days.shape[0]):
        predicted_, price_, test_day, original_log_return_, predicted_log_return = test_predicted[
            i], price[i], test_days[i], original_log_return[i], predicted_log_return[i]
        result = str(predicted_) + "," + str(price_) + "," + str(test_day) + "," + \
            str(original_log_return_) + "," + str(predicted_log_return) + "\n"
...
scaler, train_input, train_target, test_input, test_target, train_days, test_days = initialize_setting(
    window_size, prediction_horizon)

train_X = np.reshape(
    train_input, (train_input.shape[0], 1, train_input.shape[1]))
...
print_model(scaler, rnn, test_X, test_Y, test_days,
            1, window_size, prediction_horizon)
